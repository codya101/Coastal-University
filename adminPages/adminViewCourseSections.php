...
<head>
    ...
    <title>Admin View All Sections for Selected Course</title>
...
<body id="home" data-spy="scroll" data-target="#navbar-wd" data-offset="98">
    ...
    <center>
        ...
        <h1>All Sections for Fall 2022</h1>
...
<?php  
  ...
  if(isset($_POST['courseId'])){
    ...
    $sql = "SELECT Course_Section.CRN, Course_Section.sectionNum, Course.courseNum, Course.courseName, TimeSlot.Day, TimeSlot.Period, Course_Section.roomNum, Building.buildingName, Course.credits, User.firstName, User.lastName, Course_Section.availableSeats    
        FROM Course_Section
        ...
        WHERE Course_Section.courseId = $courseId AND Course_Section.semesterId='17926970'
        ORDER BY sectionNum";       
    ...
    if ($result->num_rows > 0) {
      ...
      while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["CRN"]. "</td><td>" . $row["sectionNum"]. "</td><td>" . $row["courseNum"]. "</td><td>" . $row["courseName"] . "</td><td>" . $row["Day"]. "</td><td>" .$row["Period"] . "</td><td>" .$row["roomNum"] . "</td><td>" .$row["buildingName"] . "</td><td>" .$row["credits"] . "</td><td>" .$row["firstName"] . "</td><td>" .$row["lastName"] . "</td><td>" .$row["availableSeats"] . "</td><td> <form method=POST action=adminViewRoster.php><input type=hidden name=CRN value=".$row['CRN']."><input type=submit value=Roster></form> </td></tr>" ;
      }
    } else {
      echo "0 results";
    }
...
?>
        ...
        <table border=1 height="200" width="2000" >
        <h1>All Sections for Spring 2023</h1>
        <tr style = "font-size: 28px;">
        <td bgcolor=#E69138 style="color: black;" class='medium'><strong>CRN</strong></td>
        <td bgcolor=#E69138 style="color: black;" class='medium'><strong>Section Number</strong></td>
        <td bgcolor=#E69138 style="color: black;" class='medium'><strong>Course Number</strong></td>
...
<?php  
  ...
  if(isset($_POST['courseId'])){
    ...
    $sql = "SELECT Course_Section.CRN, Course_Section.sectionNum, Course.courseNum, Course.courseName, TimeSlot.Day, TimeSlot.Period, Course_Section.roomNum, Building.buildingName, Course.credits, User.firstName, User.lastName, Course_Section.availableSeats    
        ...
        INNER JOIN Building ON Course_Section.buildingId=Building.buildingId
        INNER JOIN User ON Course_Section.facultyId=User.userId
        WHERE Course_Section.courseId = $courseId AND Course_Section.semesterId='24748568'
        ORDER BY sectionNum";       
    $result = $conn->query($sql);
