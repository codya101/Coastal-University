...
<head>
    ...
    <title>Admin View All Graduate Part Time Students</title>
...
<body id="home" data-spy="scroll" data-target="#navbar-wd" data-offset="98">
    ...
    <br>
    <center>
    <h1><strong>All Graduate Part-Time Students</strong></h1>
...
<?php
    ...
    $sql = "SELECT User.userId, Login.email, Login.password, User.firstName, User.lastName, Student.studentType, Department.departmentName
                FROM Graduate_Student
                INNER JOIN User ON Graduate_Student.studentId=User.userId
                INNER JOIN Login ON Graduate_Student.studentId=Login.userId
                INNER JOIN Student ON Graduate_Student.studentId=Student.studentId
                INNER JOIN Department ON Graduate_Student.departmentId=Department.departmentId
                WHERE Graduate_Student.graduateType= 'Part-Time'
                ORDER BY Graduate_Student.studentId"; 
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
      // output data of each row
      while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["userId"]. "</td><td>" . $row["email"]. "</td><td>" . $row["password"]. "</td><td>" . $row["firstName"]."</td><td>" . $row["lastName"]."</td><td>".$row["studentType"]. "</td><td>" .$row["departmentName"]. "</td><td>
