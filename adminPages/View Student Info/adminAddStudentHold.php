...
<head>
    ...
    <title>Admin Edit Student Hold</title>
...
<body id="home" data-spy="scroll" data-target="#navbar-wd" data-offset="98">
    ...
    <br>
    <br>
    <h1><strong>View/Add Holds</strong></h1>
    <br>
    <table border=1 height="200" width="400" >
        <tr style = "font-size: 28px;">
        <td bgcolor=#E69138 style="color: black;" class='medium'><strong>Hold Name</strong></td>
        <td bgcolor=#E69138 style="color: black;" class='medium'><strong>Add Hold</strong></td>
        </tr>
    <?php
      $sql="SELECT Hold.holdName
      FROM Student_Hold
      ...
      $result = $conn->query($sql);
          if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
              echo "</tr><td>".$row['holdName']."</td>"; 
            }
          } else {
            echo "</tr><td>This student has no holds.</td>"; 
          }
    ?>
          <td>
    <form method=POST action=adminUpdateStudentHold.php>
            <select id=holdSelect name=holdSelect> 
                <option value=selected selected=></option>
                <?php 
                    $sql = "SELECT holdId, holdName 
                        FROM Hold
                        ORDER BY holdId";       
                    $result = $conn->query($sql);
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            $row_id = $row['holdId'];  
                            echo "<option value='$row_id'>".$row['holdName']."</option> ";
                        } 
                    } else {
                      echo "0 results";
                    } 
            ...
            <input type=hidden name=studentId value='<?php echo $studentId; ?>'>
            <input type=submit value='Add Hold'> 
      </form> 
    </td></tr>
  </table>
</body>
